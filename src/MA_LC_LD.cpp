...
int PopNum;									//population size
int Mut_stren;								//mutation strength
int Max_iter;								//search depth of tabu search
int Tabu_tenure;							//tabu tenure
int NUM_LC_LD;								//size of finite sets of points considered during TS
...
{
	int i, loop, index = -1, index_first;
	double dis_min;	
	for (int i = 0; i < verticesNumber; i++)
	{
		loop = 0;
...
int add_func(int *sol, int iter, int *tbList, double **dis, int &num_p, double &objective)
{
	...
	num_p++;
	objective += delta_best;
	tbList[add_node] = iter + rand() % 10 + Tabu_tenure;
	return add_node;
}
...
double tabu_search(Clustering &currentSol)
{
	...
	while (iter < Max_iter)
	{
		...
		if (dial == 0)
		{
			...
			drop_func(sol, iter, Tb.tabuList, cluLen, clu, dis, num_p, objective);

			build_data(dis, cluLen, clu, num_p, sol);
			add_func(sol, iter, Tb.tabuList, dis, num_p, objective);
		}
...
void reformulation_local_search(Clustering &currentSolution)
{
	int flag_stop = 0;
	int iter = 0;
	double xc, xd;
	...
	while (!flag_stop &&iter<5)
	{
		...
		k_means(currentSolution, remainingPrototypes, Flag_remove);
		//cout << "here 3 " << endl;
...
void initial_population()
{
	...
	while (i <= PopNum &&difftime(endTime,startTime)<timeLimit)
	{
		...
		//cout << "here 1" << endl;
		reformulation_local_search(population[i]);
...
void mutation(Clustering &currentSolution)
{
	...
	shaking(remainingPrototypes, Mut_stren * clustersNumber, currentSolution.prototypeArray, Flag_generate);
	allocate(currentSolution);
	delete [] remainingPrototypes;
...
double memetic()
{
	...
	while (difftime(endTime, startTime)<timeLimit)
	{
		...
		reformulation_local_search(currentSolution);
...
int main(int argc, char *argv[])
{
	if (argc < 3)
	{
		exitOnError("MA_EMSSC Usage: file_name output_res_file", "(where file_name is the instance name, output_res_file is a file used to store the running information)");
	}
	char *file_name = argv[1];
	char *output_res_file = argv[2];
	...
	PopNum = 5;									//population size
	Mut_stren = 0.1;							//mutation strength
	Max_iter = 100;								//search depth of tabu search
	Tabu_tenure = 20;							//tabu tenure
	NUM_LC_LD = 20;								//the coefficient of parameter w
		
	double best_value = DBL_MAX;
	double avg_value = 0;
	double avg_time = 0;
	double temp_best = DBL_MAX;
